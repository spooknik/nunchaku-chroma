"""
This module provides the NunchakuChromaLoraLoader node
for applying LoRA weights to Nunchaku Chroma models within ComfyUI.
"""
...
class NunchakuChromaLoraLoader:
    """
    Node for loading and applying a LoRA to a Nunchaku Chroma model.
            ...
            "required": {
                "model": (
                    "MODEL",
                    {
                        ...
                        "Make sure the model is loaded by `Nunchaku Chroma DiT Loader`."
                    },
                ),
                "lora_name": (
    ...
    FUNCTION = "load_lora"
    TITLE = "Nunchaku Chroma LoRA Loader"
    ...
    def load_lora(self, model, lora_name: str, lora_strength: float):
        """
        Apply a LoRA to a Nunchaku Chroma diffusion model.
        ...
        assert isinstance(model_wrapper, ComfyChromaWrapper), (
            f"Expected ComfyChromaWrapper, got {type(model_wrapper)}. "
            "Make sure the model is loaded by `Nunchaku Chroma DiT Loader`."
        )
        ...
        assert isinstance(ret_model_wrapper, ComfyChromaWrapper)
        ...
        # Handle LoRAs that modify x_embedder (if any Chroma LoRAs do this)
        if "transformer.x_embedder.lora_A.weight" in sd:
            new_in_channels = sd["transformer.x_embedder.lora_A.weight"].shape[1]
            # Chroma packs latents, so adjust for that
...
class NunchakuChromaLoraStack:
    """
    Node for loading and applying multiple LoRAs to a Nunchaku Chroma model with dynamic input.
    ...
    TITLE = "Nunchaku Chroma LoRA Stack"
    ...
    def load_lora_stack(self, model, **kwargs):
        """
        Apply multiple LoRAs to a Nunchaku Chroma diffusion model.
